...
xtick_text = ['400000', '500000', '600000',
...
fig = go.Figure(data=[go.Scatter3d(x=X_2800, y=Y_2800, z=Z_2800,
                mode='markers', marker=dict(size=1, color=Z_2800, 
                colorscale='Spectral', colorbar=
                dict(tickvals=ztick_values_colorbar,
                ticktext=ztick_text_colorbar,title_side='right', 
                title=dict(text='Crustal Thickness Below 2800 msec',
                font=dict(size=20)),tickfont=dict(size=16))))])

'#Format Layout'
fig.update_layout(scene=dict(xaxis=dict(tickmode='array', tickvals=xtick_values, 
                  ticktext=xtick_text, title=dict(text='Easting (m)',
                  font=dict(size=18)), range=[420000, 995000], tickfont=dict(size=13)),
                  yaxis=dict(tickmode='array', tickvals=ytick_values, ticktext=ytick_text,
                  title=dict(text='Northing (m)', font=dict(size=18)),
                  range=[5180000, 5750000], tickfont=dict(size=13)), zaxis=dict(tickmode='array',
                  tickvals=ztick_values, ticktext=ztick_text,
                  title=dict(text='Thickness (msec)', font=dict(size=18)),
                  range=[0, 3000], tickfont=dict(size=13))), width=1000, height=800)
